...
#Merges to two continouus asynchron streams of id and audio together, to a chunked stream of tagged audio data
    ...
    def run(self):
        ...
        while self.id_recv.is_alive() and self.audio_recv.is_alive() and not self.please_stop.is_set():
            ...
            for i in range(len(latest_id['src'])): 
                id_info.append([latest_id['src'][i]['id'], latest_id['src'][i]['x'], latest_id['src'][i]['y'], latest_id['src'][i]['z'], latest_id['src'][i]['activity']])
